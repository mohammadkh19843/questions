...
function renderCorrectOptions() {
  ...
  options.forEach((value, index) => {
    ...
    option.value = index;
    option.textContent = value.trim() || `گزینه ${index + 1}`;
    correctOption.appendChild(option);
  });

  if (previous !== "" && Number(previous) < options.length) {
    correctOption.value = previous;
  }
}

function updateTypeUI() {
  correctBox.style.display = pollType.value === "quiz" ? "block" : "none";
}
...
pollType.addEventListener("change", updateTypeUI);
...
document.getElementById("save").onclick = async () => {
  ...
  const cleanOptions = options.map(x => x.trim());
  ...
  if (new Set(cleanOptions).size !== cleanOptions.length) {
    tg.showAlert("گزینه‌های تکراری مجاز نیستند.");
    return;
  }
  ...
  const payload = {
    ...
    type: pollType.value,
    correct_option_id:
      pollType.value === "quiz" ? Number(correctOption.value) : null,
    analysis,
    post_text: postText,
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(data.detail || "خطا در ذخیره سؤال");
    }

    result.textContent = `✅ سؤال ${data.content_id} ذخیره شد.`;
